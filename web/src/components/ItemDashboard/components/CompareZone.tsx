...
export default function CompareZone({ items, comparedSkus, onDropItem, onRemoveItem, config }: CompareZoneProps) {
  ...
  return (
    ...
    >
      ...
      {comparedItems.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-grow text-center opacity-50 pointer-events-none">
        ...
        <div className="flex flex-col h-full overflow-hidden">
          ...
          <div className="w-full text-center py-3 mb-6 bg-white/5 rounded-xl border border-white/10 text-xs uppercase tracking-widest font-bold opacity-60">
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 flex-grow items-stretch">
            {comparedItems.map((item) => (
              <div key={item.sku} className="relative flex flex-col min-w-[280px] flex-1 bg-slate-900 rounded-[1.5rem] border border-white/10 overflow-hidden shadow-xl">
                ...
                {/* Foto */}
                <div className="h-56 bg-slate-800 relative flex-shrink-0">
                  {item.image?.url && (
                ...
                <div className="p-6 flex flex-col flex-grow">
                  <h4 className="font-bold text-lg mb-6 leading-tight">{item.name}</h4>
                  
                  <div className="space-y-4 flex-grow text-sm">
                    {config.showPrice && (
                      <div className="flex justify-between items-center border-b border-white/10 pb-3">
                        <span className="opacity-50 uppercase text-[10px] font-black tracking-widest">Precio</span>
                        <span className={`font-black flex items-center gap-1 text-lg ${config.accentColor}`}>
                          <HiCurrencyEuro className="w-5 h-5" /> {item.price} €
                        </span>
                      </div>
                    )}
                    
                    <div className="flex justify-between items-center border-b border-white/10 pb-3">
                      <span className="opacity-50 uppercase text-[10px] font-black tracking-widest">Identificador</span>
                      <span className="font-mono text-xs opacity-70">{item.sku}</span>
                    </div>

                    {/* Fila de Etiquetas (Clima, Ocasión, etc.) */}
                    {item.visibleTags?.length > 0 && (
                      <div className="pt-2">
                        <span className="opacity-50 uppercase text-[10px] font-black tracking-widest block mb-3">Características</span>
                        <div className="flex flex-wrap gap-2">
                          {item.visibleTags.map((tag: string) => (
                            <span key={tag} className="bg-white/10 px-2 py-1 rounded-md text-[10px] uppercase font-bold border border-white/5">
                              {tag}
                            </span>
                          ))}
                        </div>
