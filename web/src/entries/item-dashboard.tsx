...
export default function ItemDashboard() {
  ...
  return (
    <div className="space-y-8 antialiased p-2">
      {/* 1. CABECERA DINÁMICA */}
      <div className={`bg-gradient-to-r rounded-[2.5rem] p-8 text-white shadow-2xl border-b-4 transition-all duration-500 ${
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span className={`text-[9px] font-black uppercase tracking-[0.2em] px-2 py-0.5 rounded-md border ${
                  isB2B ? 'bg-indigo-500/20 text-indigo-300 border-indigo-500/40' : 'bg-blue-500/20 text-blue-300 border-blue-500/40'
                }`}>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          <button
            ...
            className={`group relative flex flex-col h-full overflow-hidden rounded-[2.5rem] border bg-token-main-surface-secondary text-left transition-all duration-500 shadow-sm ${
              isB2B ? 'hover:shadow-[0_20px_50px_rgba(99,102,241,0.2)] border-token-border-medium hover:border-indigo-500/50' : 'hover:shadow-[0_20px_50px_rgba(37,99,235,0.15)] border-token-border-medium hover:border-blue-500/50'
            }`}
          >
            <div className="relative h-52 overflow-hidden bg-slate-900">
              ...
              <div className="absolute top-4 right-4">
                <div className={`backdrop-blur-md px-2 py-1 rounded-lg border text-[9px] font-black uppercase flex items-center gap-1 shadow-lg ${
                  isB2B ? 'bg-indigo-600/40 text-indigo-100 border-indigo-400/40' : 'bg-blue-600/40 text-blue-100 border-blue-400/40'
                }`}>
            ...
            <div className="flex flex-col flex-grow p-6">
              <h3 className={`line-clamp-2 text-lg font-black tracking-tight leading-tight transition-colors mb-2 ${
                isB2B ? 'group-hover:text-indigo-400' : 'group-hover:text-blue-600'
              }`}>
                {item.name}
              </h3>
