...
If the product cannot be clearly identified, ask a clarification question instead of guessing.`,
      ...
      inputSchema: {
        ...
        name: z.string().describe('Nombre del producto a consultar (ej: "Fibra 600Mbps")'),
      },
    },
    async ({ catalog, name }: { catalog: 'b2b' | 'b2c', name: string }) => {
      ...
      try {
        ...
        const gqlQuery = `query GetItem($id: String!, $name: String!) {
          products(
            search: $name
